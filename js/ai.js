...
export class AIService {
  ...
  getLocalizedResponse(key, params = {}) {
    ...
    const responses = {
      en: {
        ...
        priceIs: (name, price, stock) => `${name} costs ${price}. ${stock}.`,
        notFound: (name) => `I couldn't find "${name}".`,
        didYouMean: (suggestions) => `Did you mean:\n${suggestions.map(s => `• ${s}`).join('\n')}`,
      ...
      ru: {
        ...
        priceIs: (name, price, stock) => `${name} стоит ${price}. ${stock}.`,
        notFound: (name) => `Я не смог найти "${name}".`,
        didYouMean: (suggestions) => `Возможно, вы имели в виду:\n${suggestions.map(s => `• ${s}`).join('\n')}`,
  ...
  answerFromLocal(query) {
    ...
    // Helper function to format product response (localized)
    ...
    // Utility: basic similarity (token overlap)
    const tokenize = (text) => (text || '').toLowerCase().replace(/[^a-zа-яё0-9\s]/gi, ' ').split(/\s+/).filter(Boolean);
    const jaccard = (a, b) => {
    ...
    const bestMatchesByName = (needle, list, topN = 3) => {
      const tNeedle = tokenize(needle).filter(w => w.length >= 2 && !/^[0-9]+$/.test(w));
      return list
        .map(p => {
          const tokens = tokenize(p.name + ' ' + (p.name_ru || ''));
          return { p, score: jaccard(tNeedle, tokens) };
        })
        .filter(x => x.score > 0)
    ...
    // Health condition mapping (run BEFORE generic name/stock queries for better intent)
    const healthConditions = {
      'головная боль': ['Melatonin Sleep Aid', 'Aspirin Plus'],
    ...
    if (q.includes('do you have') || q.includes('есть ли') || q.includes('have') || q.includes('stock') || q.includes('наличи')) {
      ...
      const searchTerms = q
        .replace(/do you have|есть ли|in stock|в наличии|available|доступн|\?|please|пожалуйста/g, '')
        .trim()
        .split(/\s+/)
        .filter(word => {
          const stop = new Set([
            'the','a','an','any','you','and','for','about','on','in','is','are','there','have','got','some','something','anything','product','products','medicine','medicines',
            'у','нас','есть','и','для','про','на','в','ли','какие','какой','что-нибудь','чтонибудь','какое','что'
          ]);
          return word.length >= 3 && !stop.has(word.toLowerCase());
        });
    ...
    // Price queries (moved before category to prioritize exact intent)
    if (q.includes('цена') || q.includes('price') || q.includes('стоимость') || q.includes('cost')) {
      // Direct price of a named product
      const priceOfMatch = q.match(/(?:price|цена)\s*(?:of|на)?\s*(.+)/);
      if (priceOfMatch && priceOfMatch[1]) {
        const rawName = priceOfMatch[1].trim();
        const candidates = bestMatchesByName(rawName, products, 3);
        if (candidates.length > 0) {
          const target = candidates[0];
          const info = getProductInfo(target);
          return this.getLocalizedResponse('priceIs', { name: info.name, price: info.price, stock: info.stock });
        }
        const suggestionsArr = bestMatchesByName(rawName, products, 3).map(p => p.name);
        const notFound = this.getLocalizedResponse('notFound', { name: rawName });
        return suggestionsArr.length > 0 ? `${notFound}\n\n${this.getLocalizedResponse('didYouMean', { suggestions: suggestionsArr })}` : notFound;
      }
    ...
    // (moved healthConditions earlier for better intent handling)
    ...
    if (q.includes('рекоменд') || q.includes('recommend') || q.includes('посовет') || q.includes('suggest')) {
      // Prefer in-stock, highest rating if available later; for now top by stock desc
