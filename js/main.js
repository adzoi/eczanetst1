...
const translations = {
  en: {
    ...
    // Footer
    officeLocation: 'Main office: Turkey, Istanbul, Fatih',
  ...
  ru: {
    ...
    // Footer
    officeLocation: 'Главный офис: Турция, Стамбул',
...
// Test function for WhatsApp message
window.testWhatsAppMessage = function() {
  const testData = {
    name: 'Test User',
    phone: '+995 123 456 789',
    address: 'Test Address 123',
    items: 'Test Product × 2 → 10.00 GEL',
    total: '10.00 GEL'
  };
  
  console.log('Testing WhatsApp message with:', testData);
  const message = tt('whatsappOrder', testData);
...
function openWhatsAppForm(items) {
  ui.openModal((container) => {
    ...
    phoneLabel.textContent = tt('phone');
    phoneLabel.setAttribute('for', 'customer-phone');
    const phoneInput = document.createElement('input');
    phoneInput.type = 'tel';
    phoneInput.id = 'customer-phone';
    phoneInput.required = true;
    phoneInput.placeholder = tt('phonePlaceholder');
    ...
    form.addEventListener('submit', (e) => {
      ...
      redirectToWhatsApp(name, phone, address, items);
    });
...
function redirectToWhatsApp(name, phone, address, items) {
  ...
  console.log('WhatsApp message data:', { name, phone, address, items: itemsText, total: buildCurrency(total) });
  
  const message = tt('whatsappOrder', { name, phone, address, items: itemsText, total: buildCurrency(total) });
  
  // Debug: Log the final message
  console.log('Final WhatsApp message:', message);
  
  const encodedMessage = encodeURIComponent(message);
  const whatsappUrl = `https://wa.me/79283837266?text=${encodedMessage}`;
...
function initLanguageToggle() {
  ...
  const applyTranslations = () => {
    ...
    // Update footer (office location + copyright + why-us link)
    const officeLoc = document.getElementById('office-location');
    if (officeLoc) officeLoc.textContent = tt('officeLocation');
    const copyrightEl = document.getElementById('copyright-text');
    if (copyrightEl) copyrightEl.textContent = tt('copyright');
    const whyUsLink = document.getElementById('why-us-link');
    if (whyUsLink) whyUsLink.textContent = tt('whyUs');
